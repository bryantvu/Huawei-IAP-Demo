...
import com.bvutest.iapdemo.common.DeliveryUtils;
...
public class ConsumptionActivity extends Activity {
    ...
    private void deliverProduct(final String inAppPurchaseDataStr, final String inAppPurchaseDataSignature) {
        if (TextUtils.isEmpty(inAppPurchaseDataStr) || TextUtils.isEmpty(inAppPurchaseDataSignature)) {
            Utils.showMessage(this,"puchase data is error");
            return;
        }
        Log.d(TAG, "inAppPurchaseDataStr: "+ inAppPurchaseDataStr);
        Log.d(TAG, "inAppPurchaseDataSignature: "+ inAppPurchaseDataSignature);
        Log.d(TAG, "Key.getPublicKey(): "+ Key.getPublicKey());
